...
const autoCompleteListener = {
  set: async (query) => Homey.app.autoCompleteSet(query),
  state: async (query, args) => Homey.app.autoCompleteState(await getSetId(args), query)
};
...
module.exports = function initFlows() {
  ...
  registerCondition('none_active', ['set'],
    async (args) => Homey.app.getNone(await getSetId(args))
  );
  registerCondition('all_active', ['set'],
    async (args) => Homey.app.getAll(await getSetId(args))
  );
  registerCondition('is_active', ['set', 'state'],
    async (args) => Homey.app.getState(await getSetId(args), await getStateId(args))
  );
