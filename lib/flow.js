...
module.exports = {
  init(setManager) {
    ...
    flow.on('action.activate_delayed', executeAction('activate_delayed', args => setManager.setDelayed(getSetId(args), getStateId(args), args.delay)));
    ...
    flow.on('action.activate_delayed.set.autocomplete', autoCompleteSet);
    flow.on('action.activate_delayed.state.autocomplete', autoCompleteState);
