...
module.exports = {
  init(setManager) {
    ...
    function getSetId(args){
      return setManager.getSetId(args.set.name, args.set.id);
    }

    function getStateId(args){
      return setManager.getStateId(args.state.name, args.state.id);
    }
    ...
    function matchTrigger(name){
      return (callback, args, state) => {
        ...
        const stateId = args.state && getStateId(args);
        stateId && setManager.addState(setId, stateId);
