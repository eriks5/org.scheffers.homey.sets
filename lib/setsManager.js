...
function getNewId(){
  // Get a new unique id
  let id = getSettings('nextId') || 0;
  id += 1;
  updateSettings('nextId', id);
  return ""+id;
}
...
function getSetState(setId, set){
  return {
    id: setId,
    label: set.label,
    states: set.states || set.elements
  }
}
...
module.exports = {
  ...
  autoCompleteState(setId, partial){
    ...
    if (set){
      const states = getSettings('states') || {};
      const setStates = set.states || set.elements;
